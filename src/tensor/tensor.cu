...
void GPUStorage::dealloc(float *p) { cudaFree(p); }
...
void GPUStorage::add(const float *a, const float *b, float *out,
                     int64_t n) const {
...
void GPUStorage::gemm(const float *A, const float *B, const float *bias,
  ...
  cublasSgemm(cublas_handle_, CUBLAS_OP_T, CUBLAS_OP_N, N, M, K, &alpha, B,
              K,              // weight^T: K×N col-major
              A, K,           // self^T:   K×M col-major
              &beta, out, N); // out^T:    N×M col-major
...
void GPUStorage::relu(const float *in, float *out, int64_t n,
...
void CPUStorage::relu(const float *in, float *out, int64_t n,
                      cudaStream_t) const {
  for (int64_t i = 0; i < n; ++i)
    out[i] = in[i] > 0.f ? in[i] : 0.f;
}

void CPUStorage::softmax(const float *in, float *out, int rows, int cols,
                         cudaStream_t) const {
...
Tensor Tensor::clone() const {
  ...
  if (device() == Device::GPU)
    cudaMemcpy(result._data, _data, n * sizeof(float),
...
Tensor Tensor::gemm(const Tensor &weight, const Tensor &bias) const {
  ...
  _storage->gemm(data_ptr(), weight.data_ptr(), bias.data_ptr(), out.data_ptr(),
                 M, N, K);
  return out;
}
...
void Tensor::to_matlab(const std::string &path,
                       const std::string &var_name) const {
  if (_shape.size() > 2)
    throw std::runtime_error(
        "Tensor::to_matlab: only 1-D and 2-D tensors supported");

  std::ofstream f(path);
  if (!f)
    throw std::runtime_error("Tensor::to_matlab: cannot open file: " + path);
...
float Tensor::at(const std::vector<uint> &idx) const {
  if (idx.size() != _shape.size())
    throw std::out_of_range(
        "Tensor::at: number of indices does not match number of dimensions");
