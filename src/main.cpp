...
static void print_usage(const char *prog) {
  std::cerr << "Usage: " << prog
            << " -m <model.onnx> -i <images> -l <labels> [-b <batch>]\n"
            << "\n"
...
int main(int argc, char *argv[]) {
  ...
  int opt;
  while ((opt = getopt_long(argc, argv, "m:i:l:b:h", long_opts, nullptr)) !=
         -1) {
    switch (opt) {
    case 'm':
      model_path = optarg;
    ...
    case 'l':
      ...
      break;
    case 'b':
      batch_size = static_cast<size_t>(std::stoul(optarg));
      break;
    case 'h':
      print_usage(argv[0]);
      return 0;
  ...
  auto gpu_evaluator =
      tinyinfer::Evaluator::Builder().batch_size(batch_size).build();
  auto cpu_evaluator =
      tinyinfer::Evaluator::Builder().batch_size(batch_size).build();
